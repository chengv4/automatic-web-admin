...
def cookie_login():
    Cookie.set_cookie()
    try:
        WebDriverWait(Config.Browser, 5, 0.2).until(
            lambda x: x.find_element(By.CSS_SELECTOR, ".name-box")).text
...
def login():
    ...
    while True:
        ...
        if len(phone) == 11:
            break
        print("手机号码不合法！")
    input_phone = f'return document.querySelector("input[placeholder=\'手机号\']")'
    Config.Browser.execute_script(input_phone).send_keys(phone)

    # 发送验证码
    Config.Browser.find_element(By.CSS_SELECTOR,
                                'input[placeholder="验证码"] + .slot-right div > div:nth-child(2)').click()

    # 获取错误标签
    error_span = 'return document.querySelector(".css-1qf7tqh").innerHTML'
    ...
    code_input = 'return document.querySelector("input[placeholder=\'验证码\']")'
    Config.Browser.execute_script(code_input).send_keys(code)
    # 登录
    login_btn = 'return document.querySelector(\'.btn-content\')'
